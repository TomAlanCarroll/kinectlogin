...
namespace DTWGestureRecognition
{
    ...
    internal class DtwGestureRecognizer
    {
        ...
        /// </summary>
        private Dictionary<String, Dictionary<JointType, List<Vector3>>> _recordedGestures;
        ...
        public DtwGestureRecognizer(int dim, int noJoints, double recogThreshold, double posThreshold, int minLen)
        {
            ...
            _recordedGestures = new Dictionary<string, Dictionary<JointType, List<Vector3>>>();
        ...
        public void AddOrUpdate(Dictionary<JointType, List<Vector3>> skeletonTimeSequence, string lab)
        {
            _recordedGestures.Remove(lab);
            var comparer = EnumComparer<JointType>.Instance;
            //Deep copy the time Sequence
            Dictionary<JointType, List<Vector3>> seqCopy = new Dictionary<JointType, List<Vector3>>(comparer);
            foreach (JointType joint in skeletonTimeSequence.Keys)
            {
                List<Vector3> newList = new List<Vector3>();
                foreach (Vector3 v in skeletonTimeSequence[joint])
                {
                    Vector3 vClone = new Vector3(v.X, v.Y, v.Z);
                    newList.Add(vClone);
                }
        ...
        public string Recognize(Dictionary<JointType, List<Vector3>> inputSequence)
        {
            ...
            {
                foreach (JointType joint in _recordedGestures[gestureLabel].Keys)
                {
            ...
            return (minDist < _recognitionThreshold ? classification : "UNKNOWN") + " " /*+minDist.ToString()*/;
        }
        ...
        public string RetrieveText()
        {
            ...
            if (_recordedGestures != null)
            {
                foreach (string gestureName in _recordedGestures.Keys)
                {
                    ...
                    retStrBuilder.Append(newLine);
                    foreach (JointType jointName in _recordedGestures[gestureName].Keys)
                    {
                        //count skeletonData
                        ...
                        retStrBuilder.Append(newLine);
                        foreach (Vector3 pos in _recordedGestures[gestureName][jointName])
                        {
